        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="prose prose-slate max-w-none">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
              <div className="bg-[var(--color-bg-alt)] border border-[var(--color-border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--color-primary)]">50+ min</p>
                <p className="text-xs text-[var(--color-muted)] mt-1">saved every day</p>
              </div>
              <div className="bg-[var(--color-bg-alt)] border border-[var(--color-border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--color-primary)]">180+</p>
                <p className="text-xs text-[var(--color-muted)] mt-1">orders automated weekly</p>
              </div>
              <div className="bg-[var(--color-bg-alt)] border border-[var(--color-border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--color-primary)]">9</p>
                <p className="text-xs text-[var(--color-muted)] mt-1">workflows running 24/7</p>
              </div>
              <div className="bg-[var(--color-bg-alt)] border border-[var(--color-border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--color-primary)]">&pound;20k+</p>
                <p className="text-xs text-[var(--color-muted)] mt-1">estimated annual saving</p>
              </div>
            </div>
            ...
            <ol className="text-[var(--color-muted)] space-y-3 mb-4 ml-6 list-decimal">
              <li><strong className="text-[var(--color-heading)]">Order collection (2+ hours):</strong> Customers sent WhatsApp messages saying things like &ldquo;2 agege bread, 1 white bread, deliver to 14 Park Street.&rdquo; Tunmise manually copied each order into a spreadsheet. Messages were often incomplete or contradictory.</li>
              <li><strong className="text-[var(--color-heading)]">Payment tracking (2+ hours):</strong> Customers paid via SumUp (card) or bank transfer. SumUp generates its own transaction reference &mdash; breaking the link between order and payment. Tunmise cross-referenced amounts, names, and timing across two separate systems to figure out who had paid.</li>
              <li><strong className="text-[var(--color-heading)]">Production planning (1 hour):</strong> Hand-tallying how many of each bread type to bake. Get the number wrong and you overbake (waste) or underbake (unhappy customers).</li>
              <li><strong className="text-[var(--color-heading)]">Delivery planning (1 hour):</strong> Routes planned from memory. No optimisation, no grouping by area.</li>
              <li><strong className="text-[var(--color-heading)]">Customer updates (1+ hours):</strong> Manually messaging each customer to confirm their order was received and when to expect delivery.</li>
            </ol>
            ...
            <div className="space-y-4 mb-8">
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF01: Online Order Form</h3>
                <p className="text-sm text-[var(--color-body)]">Replaced WhatsApp ordering with a Tally.so form. Customers select products, enter quantities and address, and submit in under 60 seconds. No more incomplete messages.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF02: Bank Transfer Matching</h3>
                <p className="text-sm text-[var(--color-body)]">When Tunmise uploads his HSBC bank statement CSV, this workflow automatically matches bank transfers to orders using name and amount cross-referencing.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF03 &amp; WF08: SumUp Checkout Links</h3>
                <p className="text-sm text-[var(--color-body)]">Generates unique SumUp payment links for each order. Customers tap the link, pay by card, and the payment is automatically matched to their order.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF04: Production Summary</h3>
                <p className="text-sm text-[var(--color-body)]">Every morning at 8am, generates a production summary showing exactly how many of each bread type to bake. No guesswork.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF05: Delivery Route Optimiser</h3>
                <p className="text-sm text-[var(--color-body)]">Every Friday at 2pm, groups delivery addresses by town and generates Google Maps navigation links for each stop. One-click route planning.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF06: Daily Exception Alerts</h3>
                <p className="text-sm text-[var(--color-body)]">Sends a morning alert if any payments are unmatched, orders are missing information, or anything needs manual attention. Tunmise only has to look at exceptions, not every order.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF07: SumUp Transaction Matching Engine</h3>
                <p className="text-sm text-[var(--color-body)]">The most complex workflow. Polls SumUp every 30 minutes and matches transactions to orders using a 4-tier system: submission ID, name + amount, name only (partial payments), and amount only with duplicate safeguards.</p>
              </div>
              <div className="bg-[var(--color-bg-alt)]/50 border border-[var(--color-border)] rounded-xl p-4">
                <h3 className="text-sm font-semibold text-[var(--color-heading)] mb-1">WF09: Monday Cleanup</h3>
                <p className="text-sm text-[var(--color-body)]">Every Monday morning, archives the previous week&apos;s data and resets the dashboard for the new cycle. Clean slate every week.</p>
              </div>
            </div>
            ...
            <ul className="text-[var(--color-muted)] space-y-2 mb-4 ml-6 list-disc">
              <li><strong className="text-[var(--color-heading)]">Order summary:</strong> Total orders, revenue, paid vs unpaid breakdown</li>
              <li><strong className="text-[var(--color-heading)]">Unpaid customers:</strong> Names, amounts, and days since order &mdash; with a one-tap WhatsApp button to message them</li>
              <li><strong className="text-[var(--color-heading)]">Production quantities:</strong> Exactly how much of each product to bake</li>
              <li><strong className="text-[var(--color-heading)]">Delivery stops:</strong> Grouped by town with navigation links</li>
              <li><strong className="text-[var(--color-heading)]">System health:</strong> All 9 workflows showing green/amber/red status</li>
            </ul>
              ...
              <table className="w-full text-sm border border-[var(--color-border)] rounded-xl overflow-hidden">
                <thead>
                  <tr className="bg-[var(--color-surface)]/60">
                    <th className="text-left px-4 py-3 text-[var(--color-body)] font-semibold">Area</th>
                    <th className="text-left px-4 py-3 text-red-400 font-semibold">Before</th>
                    <th className="text-left px-4 py-3 text-[var(--color-success)] font-semibold">After</th>
                  </tr>
                </thead>
                <tbody className="text-[var(--color-muted)]">
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Weekly admin time</td>
                    <td className="px-4 py-3">8+ hours</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">60 seconds/day</td>
                  </tr>
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Order collection</td>
                    <td className="px-4 py-3">Manual WhatsApp messages</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">Online form with instant confirmation</td>
                  </tr>
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Payment matching</td>
                    <td className="px-4 py-3">Manual cross-referencing</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">Automated 4-tier matching engine</td>
                  </tr>
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Production planning</td>
                    <td className="px-4 py-3">Hand-tallying from messages</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">Real-time auto-generated quantities</td>
                  </tr>
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Delivery routes</td>
                    <td className="px-4 py-3">Planned from memory</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">One-click Google Maps routes</td>
                  </tr>
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Customer confirmations</td>
                    <td className="px-4 py-3">Manual WhatsApp messages</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">Instant automated via Twilio</td>
                  </tr>
                  <tr className="border-t border-[var(--color-border)]">
                    <td className="px-4 py-3 font-medium text-[var(--color-heading)]">Sales visibility</td>
                    <td className="px-4 py-3">None &mdash; guesswork</td>
                    <td className="px-4 py-3 text-[var(--color-success)]">Real-time dashboard with all financials</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <blockquote className="border-l-4 border-[var(--color-primary)] pl-4 py-2 mb-4 bg-[var(--color-bg-alt)]/50 rounded-r-lg">
              <p className="text-[var(--color-heading)] italic leading-relaxed">
            ...
            <blockquote className="border-l-4 border-[var(--color-primary)] pl-4 py-2 mb-8 bg-[var(--color-bg-alt)]/50 rounded-r-lg">
              <p className="text-[var(--color-heading)] italic leading-relaxed">
          ...
          <div className="mt-12 border border-[var(--color-border)] rounded-xl p-6">
            ...
            <ul className="space-y-3">
              <li>
                <Link href="/case-studies/emanuel-bakery" className="text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors">
              ...
              <li>
                <Link href="/services/order-automation" className="text-[var(--color-muted)] hover:text-[var(--color-heading)] transition-colors">
